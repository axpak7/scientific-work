...
for i = 1:17
    E1(i) = norm(structural_method4(f, x_start, x_end, y0, n(i), 0, 2, 2)' - solution(x_end),Inf);
    E2(i) = norm(sophisticated_structural_method4(f, x_start, x_end, y0, n(i), 0, 2, 2)' - solution(x_end),Inf);
    disp(i);
end
%E2(E2 == 0) = min(E2(E2 ~= 0));
...
hold on;
loglog(h,E2,'-o');
title('4-order structural method for system');
xlabel('h');
ylabel('|E|');
hold off;
