function y_final = structural_method4(x_0, x_n, n)
%constants
h = (x_n-x_0)/n;
x = zeros(1,n);
y1 = zeros(1,n);
...
for i = 1:n
    ...
    y1(i+1) = y1(i) + 1/10*(k11+5*k12+4*k13);
    y3(i+1) = y3(i) + 1/10*(k31+5*k32+4*k33);
    y4(i+1) = y4(i) + 1/10*(4*k41+5*k42+k43);
    y2(i+1) = y2(i) + 1/10*(4*k21+5*k22+k23);
end
